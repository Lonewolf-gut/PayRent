...
export function buildPropertyPayloadFromFormData(formData: FormData) {
  ...
  return {
    ...
    stockQuantity: parseOptionalFormNumber(formData.get("stockQuantity")) ?? 1,
    deliveryTerms: formData.get("deliveryTerms")?.toString(),
    warrantyDetails: formData.get("warrantyDetails")?.toString(),
