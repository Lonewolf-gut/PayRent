...
import { syncFinancingMandateSchema } from "@/lib/validations/financing";
...
export const POST = withAuth(
  async (req: NextRequest, _ctx, session) => {
    ...
    const parsed = syncFinancingMandateSchema.safeParse(body);
    if (!parsed.success) {
      return apiResponse({ error: parsed.error.flatten() }, 400);
    }

    const result = await financingService.syncMandateForLender(
      session.user.id,
