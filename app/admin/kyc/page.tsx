    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Review identity and bank account validation exceptions.
        </p>
      </div>
      {isLoading ? (
          ...
          <Card key={review.id}>
            <CardHeader className="flex flex-row items-start justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{review.user?.email} · {review.user?.role}</p>
              </div>
              <StatusBadge status={review.status} />
            </CardHeader>
            <CardContent>
              {review.type === "BANK" && review.data?.bankAccountId && review.status === "PENDING" && (
                <Button
                  size="sm"
