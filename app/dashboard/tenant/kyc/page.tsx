    ...
    <div className="mx-auto max-w-4xl space-y-8">
      <div>
        ...
        <p className="text-muted-foreground">
          Complete your profile, Ghana Card verification, and bank account setup.
        </p>
      </div>
        ...
        <Card>
          <CardContent className="flex items-center gap-3 pt-6">
            ...
            <div>
              ...
              <StatusBadge status={status?.profileStatus === "PROFILE_COMPLETED" ? "APPROVED" : "PENDING"} label={status?.profileStatus ?? "Incomplete"} />
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="flex items-center gap-3 pt-6">
            ...
            <div>
              ...
              <StatusBadge status={status?.kycVerified ? "APPROVED" : "PENDING"} label={status?.kycVerified ? "Verified" : "Pending"} />
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="flex items-center gap-3 pt-6">
            ...
            <div>
              ...
              <StatusBadge
                status={status?.bankAccounts?.[0]?.validationStatus === "VALIDATED" ? "APPROVED" : "PENDING"}
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <Input value={profile.employerName} onChange={(e) => setProfile({ ...profile, employerName: e.target.value })} />
          </div>
          <div>
          ...
          <div className="sm:col-span-2">
            ...
            <Input value={profile.residentialAddress} onChange={(e) => setProfile({ ...profile, residentialAddress: e.target.value })} />
          </div>
          <Button
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <Input placeholder="GHA-123456789-1" value={ghanaCard.ghanaCardNumber} onChange={(e) => setGhanaCard({ ...ghanaCard, ghanaCardNumber: e.target.value })} />
          </div>
          <div>
            <Label>Full name</Label>
            <Input value={ghanaCard.fullName} onChange={(e) => setGhanaCard({ ...ghanaCard, fullName: e.target.value })} />
          </div>
          <Button
            className="sm:col-span-2 bg-emerald-600 hover:bg-emerald-700"
            disabled={status?.kycVerified}
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2">
          ...
          <Button
            className="sm:col-span-2 bg-emerald-600 hover:bg-emerald-700"
            onClick={() => mutation.mutate({ action: "bank-account", data: { ...bank, isDefault: true } })}
          >
